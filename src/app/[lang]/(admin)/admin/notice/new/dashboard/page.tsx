...
import { createDashboardNotice } from "@/server/actions/notice/dashboard.action";
    ...
    if (file) {
      try {
        ...
        if (cloud_res.ok) {
          ...
          const { message, success } = await createDashboardNotice(
            title,
            public_id,
            secure_url,
          );
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-4xl space-y-4">
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:gap-5">
        <div>
          ...
          <Input
            ...
            name="title"
            placeholder="Title for the dashboard notice"
          />
        </div>
      </div>
      <input
      ...
      <Button disabled={loading} className="flex gap-2">
        {loading && <Loader2 className="animate-spin" />} Add Dashboard Notice
      </Button>
    </form>
  );
};
