"use server";
import Notice from "@/server/models/DashboardNotice";
...
export const createDashboardNotice = async (
  title: string,
  public_id: string,
  secure_url: string,
) => {
  const parsedData = dashboardSchema.safeParse({
    title,
    public_id,
    secure_url,
  });
...
export const deleteDashboardNotice = async (id: string) => {
